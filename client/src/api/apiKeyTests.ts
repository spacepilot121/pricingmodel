...
export type ApiKeyTestResults = { googleTest: GoogleTestResult; openAiTest: ServiceTestResult };
...
export async function testApiKeys(keys: ApiKeys): Promise<ApiKeyTestResults> {
  const [googleTest, openAiTest] = await Promise.all([testGoogleKeys(keys), testOpenAiKey(keys)]);
  return { googleTest, openAiTest };
}
