...
import { SearchResultItem } from './brandSafetyTypes.js';

export type CreatorSearchData = { identifiers: string[] };
 ...
 */
export function buildOptimisedQueryList(creatorData: CreatorSearchData): string[] {
 ...
 */
export async function callGoogle(query: string, key: string, cx: string): Promise<any> {
  try {
    ...
    if (message.toLowerCase().includes('exceeded') || reason.includes('rateLimit') || reason.includes('dailyLimit')) {
      const quotaErr: any = new Error('Google quota exhausted for today');
      quotaErr.status = 429;
      throw quotaErr;
    }
    throw err;
 ...
 */
export function dedupeResults(items: SearchResultItem[]): SearchResultItem[] {
  ...
  for (const item of items) {
    ...
    const isSnippetDuplicate = deduped.some((existing) =>
      snippetSimilarity(existing.snippet, item.snippet) >= 0.85
    );
    if (isSnippetDuplicate) continue;
