...
function normalizeEmails(values: unknown): string[] {
  ...
  const applyKeys = (obj: Record<string, unknown> | undefined | null, keys: string[]) => {
    if (!obj) return;
  ...
  rows.forEach((row) => {
    ...
    if (Array.isArray(item.contacts)) {
      item.contacts.forEach((contact) => visit((contact as Record<string, unknown>).email));
    }
    if (item.social && typeof item.social === 'object') {
      visit((item.social as Record<string, unknown>).emails);
    }
  });
